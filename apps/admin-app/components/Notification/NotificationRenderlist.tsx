...
import { useRouter } from 'next/navigation'; // 🔥 Added for programmatic routing control
...
export default function NotificationRenderList({ notifications = [] }: NotificationRenderListProps) {
  const router = useRouter(); // 🔥 Clean client routing engine instance
  ...
  const handleMarkAsReadOnly = (id: string) => {
    markRead(id, {
      onSuccess: (data) => {
      ...
      <div className="space-y-3">
        {notifications.length === 0 ? (
          ...
          notifications.map((item) => (
            <div
              key={item.id}
              // 🔥 UX Upgrade: Clicking the entire card now reads and routes asynchronously
              onClick={(e) => handleNotificationAction(item.id, item.event.link, e)}
              className="group flex gap-4 p-4 rounded-xl border bg-primary-50/20 dark:bg-primary-950/5 border-primary-100/60 dark:border-primary-950/20 shadow-sm hover:shadow-md transition-all duration-200 relative overflow-hidden cursor-pointer"
            >
              {/* Highlight Left Ribbon */}
              ...
              <div className="flex-1 min-w-0 space-y-1">
                ...
                {/* Inline Interaction link decoration */}
                <div className="pt-1">
                  <span className="inline-flex items-center gap-1 text-[11px] font-bold text-primary-500 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition">
                    View details
              ...
              {/* Separate Manual Dismiss button wrapper */}
              <div className="flex items-center self-start sm:self-center" onClick={(e) => e.stopPropagation()}>
                <button
                  onClick={() => handleMarkAsReadOnly(item.id)}
                  disabled={pendingStatus}
                  title="Mark as read"
                  className="p-2 text-secondary-400 hover:text-emerald-500 hover:bg-white dark:hover:bg-secondary-800 rounded-xl border border-transparent hover:border-secondary-100 dark:hover:border-secondary-700 shadow-none hover:shadow-sm transition active:scale-90 disabled:opacity-40"
                >
